export default function map_load_before_after(maplibregl, map) {
  fetch("road/r6noto/map/json/before_after.geojson")
  .then(function(response) {
    return response.text();
  })
  .then(function(data){
    const before_after = JSON.parse(data);

	// add markers to map
    before_after.features.forEach((marker) => {
